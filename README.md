# Introducing Bravo (beta)

Bravo is an experimental Delegated Proof of Stake blockchain with an unproven consensus
algorithm.

  - Currency symbol BRAVO
  - 1.000 BRAVO block reward at launch
  - 10% APR inflation narrowing to 1% APR over 20 years.

# Public Announcement & Discussion

Bravo was announced on the
...
* `USE_NGINX_FRONTEND` - if set to true, this will enable an NGINX reverse proxy in front of bravod that proxies websocket requests to bravod. This will also enable a custom healtcheck at the path '/health' that lists how many seconds away from current blockchain time your node is. It will return a '200' if it's less than 60 seconds away from synced.
...
* `HOME` - set this to the path where you want bravod to store it's data files (block log, shared memory, config file, etc). By default `/var/lib/bravod` is used and exists inside the docker container. If you want to use a different mountpoint (like a ramdisk, or a different drive) then you may want to set this variable to map the volume to your docker container.
...
setting `BRAVOD_SEED_NODES` in the container environment at `docker run`
