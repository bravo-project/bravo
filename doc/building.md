...
Builds bravod to be a consensus-only low memory node. Data and fields not
...
### BUILD_BRAVO_TESTNET=[OFF/ON]

Builds bravo for use in a private testnet. Also required for building unit tests.
...
We ship a Dockerfile.  This builds both common node type binaries.
    ...
    git clone https://github.com/bravocoin/bravo
    cd bravo
    docker build -t bravocoin/bravo .

## Building on Ubuntu 16.04

For Ubuntu 16.04 users, after installing the right packages with `apt` Bravo
...
Bravo requires Boost 1.58 (as in Ubuntu 16.04) and works with versions up to 1.60 (including).
So building Bravo on Ubuntu 14.04 requires downloading and installing a more recent
version of Boost.
...
Ubuntu 14.04) when compiling in C++11 mode (which Bravo does).
...
Then the instructions are the same as for bravo:
...
bravo. Until then, this will allow you to install boost 1.60.0.
...
e.g.:
    ...
    make -j$(sysctl -n hw.logicalcpu) bravod

This will only build `bravod`.
