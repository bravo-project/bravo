...
namespace bravo { namespace chain {
   ...
   class dynamic_global_property_object : public object< dynamic_global_property_object_type, dynamic_global_property_object>
   {
      public:
         ...
         asset       current_supply             = asset( 0, BRAVO_SYMBOL );
         asset       total_reward_fund_bravo    = asset( 0, BRAVO_SYMBOL );
         fc::uint128 total_reward_shares2; ///< the running total of REWARD^2

         price       get_vesting_share_price() const
         {
               return price ( asset( 1000, BRAVO_SYMBOL ), asset( 1000000, VESTS_SYMBOL ) );
         }

         price get_reward_vesting_share_price() const
         {
            return price( asset(),asset() );
         }

         uint16_t sbd_print_rate = BRAVO_100_PERCENT;
...
FC_REFLECT( bravo::chain::dynamic_global_property_object,
             (id)
             (head_block_number)
             (head_block_id)
             (time)
