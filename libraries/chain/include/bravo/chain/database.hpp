...
#include <bravo/chain/node_property_object.hpp>
#include <bravo/chain/fork_database.hpp>
...
namespace bravo { namespace chain {
   ...
   using bravo::protocol::signed_transaction;
   using bravo::protocol::operation;
   ...
   class database : public chainbase::database
   {
      public:
          ...
          */
         void open( const fc::path& data_dir, const fc::path& shared_mem_dir, uint64_t initial_supply = BRAVO_INIT_SUPPLY, uint64_t shared_file_size = 0, uint32_t chainbase_flags = 0 );
          ...
          *
          * Passing slot_num == 0 returns BRAVO_NULL_WITNESS
          */
         ...
         asset create_vesting( const account_object& to_account, asset bravo );
	 ...
	 asset get_generated_new_coins() const;
         ...
         void init_schema();
         void init_genesis(uint64_t initial_supply = BRAVO_INIT_SUPPLY );
   ...
   protected:
      ...
      private:
         ...
         fc::time_point_sec            _hardfork_times[ BRAVO_NUM_HARDFORKS + 1 ];
         protocol::hardfork_version    _hardfork_versions[ BRAVO_NUM_HARDFORKS + 1 ];
