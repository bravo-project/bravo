...
void update_witness_schedule4( database& db )
{
   ...
   active_witnesses.reserve( BRAVO_MAX_WITNESSES );
   ...
   for( auto witness_count = selected_voted.size() + selected_miners.size();
        sitr != schedule_idx.end() && witness_count < BRAVO_MAX_WITNESSES;
        ++sitr )
   {
   ...
   size_t expected_active_witnesses = std::min( size_t(BRAVO_MAX_WITNESSES), widx.size() );
   FC_ASSERT( active_witnesses.size() == expected_active_witnesses, "number of active witnesses does not equal expected_active_witnesses=${expected_active_witnesses}",
                                       ("active_witnesses.size()",active_witnesses.size()) ("BRAVO_MAX_WITNESSES",BRAVO_MAX_WITNESSES) ("expected_active_witnesses", expected_active_witnesses) );

   auto majority_version = wso.majority_version;
   if( db.has_hardfork( BRAVO_HARDFORK_0_19__1053 ) )
   {
   ...
   db.modify( wso, [&]( witness_schedule_object& _wso )
   {
      ...
      for( size_t i = active_witnesses.size(); i < BRAVO_MAX_WITNESSES; i++ )
      {
...
{
   if( (db.head_block_num() % BRAVO_MAX_WITNESSES) == 0 ) //wso.next_shuffle_block_num )
   {
