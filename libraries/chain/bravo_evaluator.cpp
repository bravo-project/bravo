...
#include <bravo/chain/witness_objects.hpp>
...
{
   FC_ASSERT( permlink.size() > BRAVO_MIN_PERMLINK_LENGTH && permlink.size() < BRAVO_MAX_PERMLINK_LENGTH, "Permlink is not a valid size." );
...
void witness_update_evaluator::do_apply( const witness_update_operation& o )
{
   ...
   FC_ASSERT( o.url.size() <= BRAVO_MAX_WITNESS_URL_LENGTH, "URL is too long" );
   

   FC_ASSERT( o.props.account_creation_fee.symbol == BRAVO_SYMBOL );
...
void account_create_evaluator::do_apply( const account_create_operation& o )
{
   ...
   FC_ASSERT( o.fee >= asset( wso.median_props.account_creation_fee.amount * BRAVO_CREATE_ACCOUNT_WITH_MODIFIER, BRAVO_SYMBOL ), 
      "Insufficient Fee: ${f} required, ${p} provided.",
      ("f", asset( wso.median_props.account_creation_fee.amount * BRAVO_CREATE_ACCOUNT_WITH_MODIFIER, BRAVO_SYMBOL ) )
      ("p", o.fee) );
   ...
   if( o.fee.amount > 0 ) {
      //TODO Add balance to new use and reward fund
	   auto new_account_share = (o.fee.amount * BRAVO_NEW_ACCOUNT_CREATION_SHARE) / BRAVO_100_PERCENT;;
	   auto reward_share = o.fee.amount - new_account_share;
	   
       _db.create_vesting(new_account, asset(new_account_share, BRAVO_SYMBOL) );
       _db.pay_reward_funds(reward_share);
   }
}

void account_update_evaluator::do_apply( const account_update_operation& o )
{
   FC_ASSERT( o.account != BRAVO_TEMP_ACCOUNT, "Cannot update temp account." );
   ...
   {
#ifndef IS_TEST_NET
      FC_ASSERT( _db.head_block_time() - account_auth.last_owner_update > BRAVO_OWNER_UPDATE_LIMIT, "Owner authority can only be updated once an hour." );
#endif
...
void comment_options_evaluator::do_apply( const comment_options_operation& o )
{
   ...
   FC_ASSERT( comment.percent_bravo_dollars >= o.percent_bravo_dollars, "A comment cannot accept a greater percent SBD." );

   _db.modify( comment, [&]( comment_object& c ) {
       c.max_accepted_payout   = o.max_accepted_payout;
       c.percent_bravo_dollars = o.percent_bravo_dollars;
...
{ try {
   ...
   if( o.parent_author != BRAVO_ROOT_POST_PARENT )
   {
      parent = &_db.get_comment( o.parent_author, o.parent_permlink );
      FC_ASSERT( parent->depth < BRAVO_MAX_COMMENT_DEPTH, "Comment is nested ${x} posts deep, maximum depth is ${y}.", ("x",parent->depth)("y",BRAVO_MAX_COMMENT_DEPTH) );
   }
   ...
   if ( itr == by_permlink_idx.end() )
   {
      ...
      if( o.parent_author == BRAVO_ROOT_POST_PARENT )
          FC_ASSERT( ( now - auth.last_root_post ) > BRAVO_MIN_ROOT_COMMENT_INTERVAL, "You may only post once every 5 minutes.", ("now",now)("last_root_post", auth.last_root_post) );
      else
          FC_ASSERT( (now - auth.last_post) > BRAVO_MIN_REPLY_INTERVAL, "You may only comment once every 20 seconds.", ("now",now)("auth.last_post",auth.last_post) );

      uint16_t reward_weight = BRAVO_100_PERCENT;
      ...
      const auto& new_comment = _db.create< comment_object >( [&]( comment_object& com )
      {
         ...
         com.cashout_time = com.created + BRAVO_CASHOUT_WINDOW_SECONDS;
         com.max_cashout_windows = BRAVO_MAX_CASHOUT_WINDOWS;
...
void escrow_transfer_evaluator::do_apply( const escrow_transfer_operation& o )
{
   try
   {
      ...
      asset bravo_spent = o.bravo_amount;
       
      if( o.fee.symbol == BRAVO_SYMBOL )
         bravo_spent += o.fee;
       
      FC_ASSERT( from_account.balance >= bravo_spent, "Account cannot cover BRAVO costs of escrow. Required: ${r} Available: ${a}", ("r",bravo_spent)("a",from_account.balance) );
      
      _db.adjust_balance( from_account, -bravo_spent );
      _db.create<escrow_object>([&]( escrow_object& esc )
      {
         ...
         esc.bravo_balance          = o.bravo_amount;
...
void escrow_approve_evaluator::do_apply( const escrow_approve_operation& o )
{
   try
   {
      ...
      {
         const auto& from_account = _db.get_account( o.from );
         _db.adjust_balance( from_account, escrow.bravo_balance );
...
void escrow_release_evaluator::do_apply( const escrow_release_operation& o )
{
   try
   {
      ...
      const auto& e = _db.get_escrow( o.from, o.escrow_id );
      FC_ASSERT( e.bravo_balance >= o.bravo_amount, "Release amount exceeds escrow balance. Amount: ${a}, Balance: ${b}", ("a", o.bravo_amount)("b", e.bravo_balance) );
      FC_ASSERT( e.to == o.to, "Operation 'to' (${o}) does not match escrow 'to' (${e}).", ("o", o.to)("e", e.to) );
      FC_ASSERT( e.agent == o.agent, "Operation 'agent' (${a}) does not match escrow 'agent' (${e}).", ("o", o.agent)("e", e.agent) );
      ...
      _db.adjust_balance( receiver_account, o.bravo_amount );
      _db.modify( e, [&]( escrow_object& esc )
      {
         esc.bravo_balance -= o.bravo_amount;
      });

      if( e.bravo_balance.amount == 0 )
      {
         _db.remove( e );
      }
   }
...
void transfer_to_vesting_evaluator::do_apply( const transfer_to_vesting_operation& o )
{
   ...
   FC_ASSERT( _db.get_balance( from_account, BRAVO_SYMBOL) >= o.amount, "Account does not have sufficient BRAVO for transfer." );
   _db.adjust_balance( from_account, -o.amount );
...
void set_withdraw_vesting_route_evaluator::do_apply( const set_withdraw_vesting_route_operation& o )
{
   ...
   if( itr == wd_idx.end() )
   {
      ...
      FC_ASSERT( from_account.withdraw_routes < BRAVO_MAX_WITHDRAW_ROUTES, "Account already has the maximum number of routes." );
   ...
   FC_ASSERT( total_percent <= BRAVO_100_PERCENT, "More than 100% of vesting withdrawals allocated to destinations." );
   }
...
void account_witness_proxy_evaluator::do_apply( const account_witness_proxy_operation& o )
{
   ...
   if( o.proxy.size() ) {
      ...
      proxy_chain.reserve( BRAVO_MAX_PROXY_RECURSION_DEPTH + 1 );
      ...
      while( cprox->proxy.size() != 0 ) {
         ...
         cprox = &next_proxy;
         FC_ASSERT( proxy_chain.size() <= BRAVO_MAX_PROXY_RECURSION_DEPTH, "Proxy chain is too long." );
      }
...
void account_witness_vote_evaluator::do_apply( const account_witness_vote_operation& o )
{
   ...
   if( itr == by_account_witness_idx.end() ) {
      FC_ASSERT( o.approve, "Vote doesn't exist, user must indicate a desire to approve witness." );
         ...
         FC_ASSERT( voter.witnesses_voted_for < BRAVO_MAX_ACCOUNT_WITNESS_VOTES, "Account has voted for too many witnesses." ); // TODO: Remove after hardfork 2
...
{ try {
   ...
   FC_ASSERT( elapsed_seconds >= BRAVO_MIN_VOTE_INTERVAL_SEC, "Can only vote once every 3 seconds." );

   int64_t regenerated_power = (BRAVO_100_PERCENT * elapsed_seconds) / BRAVO_VOTE_REGENERATION_SECONDS;
   int64_t current_power     = std::min( int64_t(voter.voting_power + regenerated_power), int64_t(BRAVO_100_PERCENT) );
   FC_ASSERT( current_power > 0, "Account currently does not have voting power." );

   int64_t  abs_weight    = abs(o.weight);
   int64_t  used_power    = (current_power * abs_weight) / BRAVO_100_PERCENT;

   const dynamic_global_property_object& dgpo = _db.get_dynamic_global_properties();

   // used_power = (current_power * abs_weight / BRAVO_100_PERCENT) * (reserve / max_vote_denom)
   // The second multiplication is rounded up as of HF 259
   int64_t max_vote_denom = dgpo.vote_power_reserve_rate * BRAVO_VOTE_REGENERATION_SECONDS / (60*60*24);
   FC_ASSERT( max_vote_denom > 0 );

   used_power = (used_power + max_vote_denom - 1) / max_vote_denom;
   FC_ASSERT( used_power <= current_power, "Account does not have enough power to vote." );

   int64_t abs_rshares = ((uint128_t(voter.effective_bravo_balance().amount.value) * used_power) / (BRAVO_100_PERCENT)).to_uint64();
	
   // Need explaination of BRAVO_VOTE_DUST_THRESHOLD 
   //FC_ASSERT( abs_rshares > BRAVO_VOTE_DUST_THRESHOLD || o.weight == 0, "Voting weight is too small, please accumulate more voting power or bravo." );
   FC_ASSERT( abs_rshares > 0 || o.weight == 0, "Voting weight is too small, please accumulate more voting power or bravo." );
   ...
   if( itr == comment_vote_idx.end() )
   {
      ...
      if( rshares > 0 )
      {
         FC_ASSERT( _db.head_block_time() < comment.cashout_time - BRAVO_UPVOTE_LOCKOUT_HF17, "Cannot increase payout within last twelve hours before payout." );
      }
      ...
      **/
      _db.create<comment_vote_object>( [&]( comment_vote_object& cv ){
         ...
         if( curation_reward_eligible )
         {
            if( comment.created < fc::time_point_sec(BRAVO_HARDFORK_0_6_REVERSE_AUCTION_TIME) ) {
            ...
            if( _db.head_block_time() > fc::time_point_sec(BRAVO_HARDFORK_0_6_REVERSE_AUCTION_TIME) )  /// start enforcing this prior to the hardfork
            {
               /// discount weight by time
               uint128_t w(max_vote_weight);
               uint64_t delta_t = std::min( uint64_t((cv.last_update - comment.created).to_seconds()), uint64_t(BRAVO_REVERSE_AUCTION_WINDOW_SECONDS) );

               w *= delta_t;
               w /= BRAVO_REVERSE_AUCTION_WINDOW_SECONDS;
   ...
   {
      FC_ASSERT( itr->num_changes < BRAVO_MAX_VOTE_CHANGES, "Voter has used the maximum number of vote changes on this comment." );
...
void pow_apply( database& db, Operation o )
{
   ...
   if( db.head_block_num() < BRAVO_START_MINER_VOTING_BLOCK )
      pow_reward.amount *= BRAVO_MAX_WITNESSES;
...
void pow2_evaluator::do_apply( const pow2_operation& o )
{
   ...
   FC_ASSERT( o.props.maximum_block_size >= BRAVO_MIN_BLOCK_SIZE_LIMIT * 2, "Voted maximum block size is too small." );
...
void convert_evaluator::do_apply( const convert_operation& o )
{
  ...
  auto bravo_conversion_delay = BRAVO_CONVERSION_DELAY;
  ...
  _db.create<convert_request_object>( [&]( convert_request_object& obj )
  {
      ...
      obj.amount          = o.amount;
      obj.conversion_date = _db.head_block_time() + bravo_conversion_delay;
  });
...
void challenge_authority_evaluator::do_apply( const challenge_authority_operation& o )
{
   ...
   if( o.require_owner )
   {
      ...
      FC_ASSERT( challenger.balance >= BRAVO_OWNER_CHALLENGE_FEE );
      FC_ASSERT( !challenged.owner_challenged );
      FC_ASSERT( _db.head_block_time() - challenged.last_owner_proved > BRAVO_OWNER_CHALLENGE_COOLDOWN );

      _db.adjust_balance( challenger, - BRAVO_OWNER_CHALLENGE_FEE );
      _db.create_vesting( _db.get_account( o.challenged ), BRAVO_OWNER_CHALLENGE_FEE );
  ...
  {
      FC_ASSERT( challenger.balance >= BRAVO_ACTIVE_CHALLENGE_FEE, "Account does not have sufficient funds to pay challenge fee." );
      FC_ASSERT( !( challenged.owner_challenged || challenged.active_challenged ), "Account is already challenged." );
      FC_ASSERT( _db.head_block_time() - challenged.last_active_proved > BRAVO_ACTIVE_CHALLENGE_COOLDOWN, "Account cannot be challenged because it was recently challenged." );

      _db.adjust_balance( challenger, - BRAVO_ACTIVE_CHALLENGE_FEE );
      _db.create_vesting( _db.get_account( o.challenged ), BRAVO_ACTIVE_CHALLENGE_FEE );
...
void request_account_recovery_evaluator::do_apply( const request_account_recovery_operation& o )
{
   ...
   if( request == recovery_request_idx.end() ) // New Request
   {
      ...
      _db.create< account_recovery_request_object >( [&]( account_recovery_request_object& req )
      {
         ...
         req.expires = _db.head_block_time() + BRAVO_ACCOUNT_RECOVERY_REQUEST_EXPIRATION_PERIOD;
      });
   }
...
void recover_account_evaluator::do_apply( const recover_account_operation& o )
{
  ...
  FC_ASSERT( _db.head_block_time() - account.last_account_recovery > BRAVO_OWNER_UPDATE_LIMIT, "Owner authority can only be updated once an hour." );
...
void change_recovery_account_evaluator::do_apply( const change_recovery_account_operation& o )
{
   ...
   if( request == change_recovery_idx.end() ) // New request
   {
      _db.create< change_recovery_account_request_object >( [&]( change_recovery_account_request_object& req )
      {
         ...
         req.effective_on = _db.head_block_time() + BRAVO_OWNER_AUTH_RECOVERY_PERIOD;
      });
   }
...
void transfer_from_savings_evaluator::do_apply( const transfer_from_savings_operation& op )
{
   ...
   FC_ASSERT( from.savings_withdraw_requests < BRAVO_SAVINGS_WITHDRAW_REQUEST_LIMIT, "Account has reached limit for pending withdraw requests." );
   ...
   _db.create<savings_withdraw_object>( [&]( savings_withdraw_object& s ) {
      ...
      s.complete = _db.head_block_time() + BRAVO_SAVINGS_WITHDRAW_TIME;
   });
...
void decline_voting_rights_evaluator::do_apply( const decline_voting_rights_operation& o )
{
   ...
   if( o.decline )
   {
      ...
      _db.create< decline_voting_rights_request_object >( [&]( decline_voting_rights_request_object& req )
      {
         ...
         req.effective_date = _db.head_block_time() + BRAVO_OWNER_AUTH_RECOVERY_PERIOD;
      });
   }
   else
   {
...
void claim_reward_balance_evaluator::do_apply( const claim_reward_balance_operation& op )
{
   ...
   FC_ASSERT( op.reward_bravo <= acnt.reward_bravo_balance, "Cannot claim that much BRAVO. Claim: ${c} Actual: ${a}",
      ("c", op.reward_bravo)("a", acnt.reward_bravo_balance) );
   
   _db.adjust_reward_balance( acnt, -op.reward_bravo );
   _db.adjust_balance( acnt, op.reward_bravo );
}
