...
namespace bravo { namespace protocol {
   ...
   void account_create_operation::validate() const
   {
      ...
      FC_ASSERT( is_asset_type( fee, BRAVO_SYMBOL ), "Account creation fee must be BRAVO" );
      owner.validate();
      active.validate();
      ...
      FC_ASSERT( fee >= asset( 0, BRAVO_SYMBOL ), "Account creation fee cannot be negative" );
   }
   ...
   void comment_payout_beneficiaries::validate()const
   {
      ...
      FC_ASSERT( beneficiaries[0].weight <= BRAVO_100_PERCENT, "Cannot allocate more than 100% of rewards to one account" );
      sum += beneficiaries[0].weight;
      FC_ASSERT( sum <= BRAVO_100_PERCENT, "Cannot allocate more than 100% of rewards to a comment" ); // Have to check incrementally to avoid overflow

      for( size_t i = 1; i < beneficiaries.size(); i++ )
      {
         validate_account_name( beneficiaries[i].account );
         FC_ASSERT( beneficiaries[i].weight <= BRAVO_100_PERCENT, "Cannot allocate more than 100% of rewards to one account" );
   ...
   {
      validate_account_name( author );
      FC_ASSERT( percent_bravo_dollars <= BRAVO_100_PERCENT, "Percent cannot exceed 100%" );
      FC_ASSERT( max_accepted_payout.symbol == BRAVO_SYMBOL, "Max accepted payout must be in BRAVO" );
   ...
   void vote_operation::validate() const
   {
      ...
      validate_account_name( author );\
      FC_ASSERT( abs(weight) <= BRAVO_100_PERCENT, "Weight is not a BRAVO percentage" );
      validate_permlink( permlink );
   }
   ...
   { try {
      ...
      FC_ASSERT( amount.symbol != VESTS_SYMBOL, "transferring of Bravo Power (STMP) is not allowed." );
   ...
   {
      validate_account_name( from );
      FC_ASSERT( is_asset_type( amount, BRAVO_SYMBOL ), "Amount must be BRAVO" );
      if ( to != account_name_type() ) validate_account_name( to );
      FC_ASSERT( amount > asset( 0, BRAVO_SYMBOL ), "Must transfer a nonzero amount" );
   }
   ...
   void set_withdraw_vesting_route_operation::validate() const
   {
      ...
      FC_ASSERT( 0 <= percent && percent <= BRAVO_100_PERCENT, "Percent must be valid bravo percent" );
   }
   ...
   void witness_update_operation::validate() const
   {
      ...
      FC_ASSERT( fee >= asset( 0, BRAVO_SYMBOL ), "Fee cannot be negative" );
      props.validate();
   }
   ...
   void equihash_pow::create( const block_id_type& recent_block, const account_name_type& account_name, uint32_t nonce )
   {
      ...
      proof = fc::equihash::proof::hash( BRAVO_EQUIHASH_N, BRAVO_EQUIHASH_K, seed );
   ...
   void equihash_pow::validate() const
   {
      validate_account_name( input.worker_account );
      auto seed = fc::sha256::hash( input );
      FC_ASSERT( proof.n == BRAVO_EQUIHASH_N, "proof of work 'n' value is incorrect" );
      FC_ASSERT( proof.k == BRAVO_EQUIHASH_K, "proof of work 'k' value is incorrect" );
   ...
   void escrow_release_operation::validate()const
   {
      ...
      FC_ASSERT( bravo_amount.amount >= 0, "bravo amount cannot be negative" );
      FC_ASSERT( bravo_amount.amount > 0, "escrow must release a non-zero amount" );
      FC_ASSERT( bravo_amount.symbol == BRAVO_SYMBOL, "bravo amount must contain BRAVO" );
   }
   ...
   {
      validate_account_name( account );
      FC_ASSERT( is_asset_type( reward_bravo, BRAVO_SYMBOL ), "Reward Bravo must be BRAVO" );
      FC_ASSERT( reward_bravo.amount >= 0, "Cannot claim a negative amount" );
      FC_ASSERT( reward_bravo.amount > 0, "Must claim something." );
   }
