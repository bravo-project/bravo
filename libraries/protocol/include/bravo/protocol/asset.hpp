...
namespace bravo { namespace protocol {
   ...
   struct asset
   {
      asset( share_type a = 0, asset_symbol_type id = BRAVO_SYMBOL )
      :amount(a),symbol(id){}
...
namespace fc {
    inline void to_variant( const bravo::protocol::asset& var,  fc::variant& vo ) { vo = var.to_string(); }
    inline void from_variant( const fc::variant& var,  bravo::protocol::asset& vo ) { vo = bravo::protocol::asset::from_string( var.as_string() ); }
}

FC_REFLECT( bravo::protocol::asset, (amount)(symbol) )
FC_REFLECT( bravo::protocol::price, (base)(quote) )
