...
#include <bravo/app/database_api.hpp>
...
{
   public:
      database_api_impl( const bravo::app::api_context& ctx  );
      ~database_api_impl();
      ...
      bravo::chain::database&                _db;
      std::shared_ptr< bravo::follow::follow_api > _follow_api;
...
database_api::database_api( const bravo::app::api_context& ctx )
   : my( new database_api_impl( ctx ) ) {}

database_api::~database_api() {}

database_api_impl::database_api_impl( const bravo::app::api_context& ctx )
   : _db( *ctx.app.chain_database() )
{
   ...
   try
   {
      ...
      _follow_api = std::make_shared< bravo::follow::follow_api >( ctx );
   }
...
{
   return bravo::protocol::get_config();
}
...
vector<account_id_type> database_api_impl::get_account_references( account_id_type account_id )const
{
   ...
   const auto& refs = aidx.get_secondary_index<bravo::chain::account_member_index>();
   ...
   return result;*/
   FC_ASSERT( false, "database_api::get_account_references --- Needs to be refactored for bravo." );
}
...
//   wdump((trx)(available_keys));
   auto result = trx.get_required_signatures( BRAVO_CHAIN_ID,
                                              available_keys,
...
{
   trx.verify_authority( BRAVO_CHAIN_ID,
                         [&]( string account_name ){ return authority( _db.get< account_authority_object, by_account >( account_name ).active  ); },
                         [&]( string account_name ){ return authority( _db.get< account_authority_object, by_account >( account_name ).owner   ); },
                         [&]( string account_name ){ return authority( _db.get< account_authority_object, by_account >( account_name ).posting ); },
                         BRAVO_MAX_SIG_CHECK_DEPTH );
...
void database_api::set_pending_payout( discussion& d )const
{
   ...
   if( itr != cidx.end() && itr->comment == d.id )  {
      d.promoted = asset( itr->promoted_balance, BRAVO_SYMBOL );
   }
   ...
   if( total_r2 > 0 )
   {
      ...
      vshares = d.net_rshares.value > 0 ? bravo::chain::util::evaluate_reward_curve( d.net_rshares.value, rf.author_reward_curve, rf.content_constant ) : 0;
   ...
   if( d.parent_author != BRAVO_ROOT_POST_PARENT )
                                                  ...
                                                  )const
{
   ...
   while( count > 0 && tidx_itr != tidx.end() )
   {
      ...
      try
      {
         ...
         result.back().promoted = asset(tidx_itr->promoted_balance, BRAVO_SYMBOL );
...
vector<discussion> database_api::get_discussions_by_promoted( const discussion_query& query )const
{
   return my->_db.with_read_lock( [&]()
   {
      ...
      auto tidx_itr = tidx.lower_bound( boost::make_tuple( tag, parent, share_type(BRAVO_MAX_SHARE_SUPPLY) )  );
