#include <bravo/market_history/market_history_api.hpp>
...
{
   public:
      market_history_api_impl( bravo::app::application& _app )
         :app( _app ) {}
...
market_ticker market_history_api_impl::get_ticker() const
{
   ...
   {
      auto open = ( asset( itr->open_sbd, BBD_SYMBOL ) / asset( itr->open_bravo, BRAVO_SYMBOL ) ).to_real();
      result.latest = ( asset( itr->close_sbd, BBD_SYMBOL ) / asset( itr->close_bravo, BRAVO_SYMBOL ) ).to_real();
   ...
   result.bravo_volume = volume.bravo_volume;
...
market_volume market_history_api_impl::get_volume() const
{
   ...
   do
   {
      result.bravo_volume.amount += itr->bravo_volume;
...
order_book market_history_api_impl::get_order_book( uint32_t limit ) const
{
   ...
   const auto& order_idx = app.chain_database()->get_index< bravo::chain::limit_order_index >().indices().get< bravo::chain::by_price >();
   auto itr = order_idx.lower_bound( price::max( BBD_SYMBOL, BRAVO_SYMBOL ) );
   ...
   while( itr != order_idx.end() && itr->sell_price.base.symbol == BBD_SYMBOL && result.bids.size() < limit )
   {
      ...
      cur.bravo = ( asset( itr->for_sale, BBD_SYMBOL ) * itr->sell_price ).amount;
      cur.sbd = itr->for_sale;
      result.bids.push_back( cur );
      ++itr;
   }

   itr = order_idx.lower_bound( price::max( BRAVO_SYMBOL, BBD_SYMBOL ) );

   while( itr != order_idx.end() && itr->sell_price.base.symbol == BRAVO_SYMBOL && result.asks.size() < limit )
   {
      order cur;
      cur.price = itr->sell_price.quote.to_real() / itr->sell_price.base.to_real();
      cur.bravo = itr->for_sale;
      cur.sbd = ( asset( itr->for_sale, BRAVO_SYMBOL ) * itr->sell_price ).amount;
...
market_history_api::market_history_api( const bravo::app::api_context& ctx )
{
