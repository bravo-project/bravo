...
#define BRAVO_NAMESPACE_PREFIX "bravo::protocol::"

namespace bravo { namespace account_history {
...
void account_history_plugin::plugin_initialize(const boost::program_options::variables_map& options)
{
   ...
   if( options.count( "history-whitelist-ops" ) )
   {
      ...
      for( auto& arg : options.at( "history-whitelist-ops" ).as< vector< string > >() )
      {
         ...
         {
            if( op.size() )
               my->_op_list.insert( BRAVO_NAMESPACE_PREFIX + op );
         }
      }
...
BRAVO_DEFINE_PLUGIN( account_history, bravo::account_history::account_history_plugin )
