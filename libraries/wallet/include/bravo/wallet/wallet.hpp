...
#include <bravo/follow/follow_plugin.hpp>
#include <bravo/app/bravo_api_objects.hpp>
...
using bravo::app::discussion;
using namespace bravo::private_message;
...
class wallet_api
{
   public:
       ...
       *  'info' wallet command.
       *
       *  These accounts are created with combination of BRAVO and delegated SP
       *
       *  @param creator The account creating the new account
       *  @param bravo_fee The amount of the fee to be paid with BRAVO
       *  @param delegated_vests The amount of the fee to be paid with delegation
       *  @param new_account_name The name of the new account
       *  @param json_meta JSON Metadata associated with the new account
       *  @param broadcast true if you wish to broadcast the transaction
       */
      annotated_signed_transaction create_account_delegated( string creator, asset bravo_fee, asset delegated_vests, string new_account_name, string json_meta, bool broadcast );
      ...
      /**
       * Transfer funds from one account to another. BRAVO and SBD can be transferred.
       *
       * @param from The account the funds are coming from
       * @param to The account the funds are going to
       * @param amount The funds being transferred. i.e. "100.000 BRAVO"
      ...
      /**
       * Transfer funds from one account to another using escrow. BRAVO and SBD can be transferred.
       *
       * @param from The account the funds are coming from
       * @param to The account the funds are going to
       * @param agent The account acting as the agent in case of dispute
       * @param escrow_id A unique id for the escrow transfer. (from, escrow_id) must be a unique pair
       * @param bravo_amount The amount of BRAVO to transfer
       ...
       * @param bravo_amount The amount of BRAVO that will be released
       * @param broadcast true if you wish to broadcast the transaction
       */
       ...
       * @param to   The account receiving either VESTS or BRAVO.
      ...
      /**
       *  This method will convert SBD to BRAVO at the current_median_history price one
       ...
       *                       operations defined in `bravo/chain/operations.hpp`
      ...
      /**
       * Gets the current order book for BRAVO:SBD
       *
       ...
       *  @param amount_to_sell The amount of either SBD or BRAVO you wish to sell
      ...
      /**
       * Vote on a comment to be paid BRAVO
       *
      ...
      /**
       * Challenge a user's authority. The challenger pays a fee to the challenged which is deposited as
       * Bravo Power. Until the challenged proves their active key, all posting rights are revoked.
       *
      ...
      annotated_signed_transaction claim_reward_balance( string account, asset reward_bravo, bool broadcast );
};
...
FC_REFLECT( bravo::wallet::wallet_data,
            (cipher_keys)
            (ws_server)
            (ws_user)
            (ws_password)
...
FC_REFLECT( bravo::wallet::brain_key_info, (brain_priv_key)(wif_priv_key) (pub_key))

FC_REFLECT( bravo::wallet::plain_keys, (checksum)(keys) )

FC_REFLECT_ENUM( bravo::wallet::authority_type, (owner)(active)(posting) )

FC_API( bravo::wallet::wallet_api,
        /// wallet api
        (help)(gethelp)
...
FC_REFLECT( bravo::wallet::memo_data, (from)(to)(nonce)(check)(encrypted) )
