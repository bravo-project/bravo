...
#include <bravo/witness/witness_objects.hpp>
...
BOOST_AUTO_TEST_CASE( account_create_apply )
{
   try
   {
      ...
      const account_object& init = db.get_account( BRAVO_INIT_MINER_NAME );
      ...
      op.fee = asset( 100, BRAVO_SYMBOL );
      op.new_account_name = "alice";
      op.creator = BRAVO_INIT_MINER_NAME;
      ...
      auto new_acc_share = (op.fee.amount * BRAVO_NEW_ACCOUNT_CREATION_SHARE) / BRAVO_100_PERCENT;
      auto new_acc_reward_share = op.fee.amount - new_acc_share;
      BOOST_REQUIRE( acct.balance.amount.value == ASSET( "0.000 TESTS" ).amount.value );
      BOOST_REQUIRE( acct.reward_bravo_balance.amount.value == new_acc_share.value );
      BOOST_REQUIRE( acct.id._id == acct_auth.id._id );

      /// because init_witness has created vesting shares and blocks have been produced, 100 BRAVO is worth less than 100 vesting shares due to rounding
      ...
      BOOST_REQUIRE( acct.proxy == "" );
      BOOST_REQUIRE( acct.created == db.head_block_time() );
      BOOST_REQUIRE( acct.balance.amount.value == ASSET( "0.000 BRAVO " ).amount.value );
      //BOOST_REQUIRE( acct.vesting_shares.amount.value == ( op.fee * ( vest_shares / vests ) ).amount.value );
      //BOOST_REQUIRE( acct.vesting_withdraw_rate.amount.value == ASSET( "0.000000 VESTS" ).amount.value );
      //BOOST_REQUIRE( acct.proxied_vsf_votes_total().value == 0 );
      //BOOST_REQUIRE( ( init_starting_balance - ASSET( "0.100 TESTS" ) ).amount.value == init.balance.amount.value );
      ...
      tx.signatures.clear();
      tx.operations.clear();
      op.fee = asset( db.get_account( BRAVO_INIT_MINER_NAME ).balance.amount + 1, BRAVO_SYMBOL );
      op.new_account_name = "bob";
      tx.operations.push_back( op );
      tx.sign( init_account_priv_key, db.get_chain_id() );
...
BOOST_AUTO_TEST_CASE( account_update_authorities )
{
   try
   {
      ...
      BRAVO_REQUIRE_THROW( db.push_transaction( tx, 0 ), tx_missing_active_auth );
      ...
      tx.sign( alice_private_key, db.get_chain_id() );
      BRAVO_REQUIRE_THROW( db.push_transaction( tx, 0 ), tx_irrelevant_sig );

      BOOST_TEST_MESSAGE( "--- Test failure when containing duplicate signatures" );
      tx.signatures.clear();
      tx.sign( active_key, db.get_chain_id() );
      tx.sign( active_key, db.get_chain_id() );
      BRAVO_REQUIRE_THROW( db.push_transaction( tx, 0 ), tx_duplicate_sig );
      ...
      tx.operations.push_back( op );
      tx.sign( active_key, db.get_chain_id() );
      BRAVO_REQUIRE_THROW( db.push_transaction( tx, 0 ), tx_missing_owner_auth );
...
BOOST_AUTO_TEST_CASE( comment_authorities )
{
   try
   {
      ...
      ACTORS( (alice)(bob) );
      generate_blocks( 60 / BRAVO_BLOCK_INTERVAL );
      ...
      BRAVO_REQUIRE_THROW( db.push_transaction( tx, 0 ), tx_missing_posting_auth );
      ...
      tx.signatures.clear();
      tx.sign( bob_private_key, db.get_chain_id() );
      BRAVO_REQUIRE_THROW( db.push_transaction( tx, database::skip_transaction_dupe_check ), tx_missing_posting_auth );
...
BOOST_AUTO_TEST_CASE( comment_apply )
{
   try
   {
      ...
      BOOST_REQUIRE( alice_comment.cashout_time == fc::time_point_sec( db.head_block_time() + fc::seconds( BRAVO_CASHOUT_WINDOW_SECONDS ) ) );
      ...
      BOOST_REQUIRE( bob_comment.cashout_time == bob_comment.created + BRAVO_CASHOUT_WINDOW_SECONDS );
      ...
      BOOST_REQUIRE( sam_comment.cashout_time == sam_comment.created + BRAVO_CASHOUT_WINDOW_SECONDS );
      BOOST_REQUIRE( sam_comment.root_comment == alice_comment.id );
      validate_database();

      generate_blocks( 60 * 5 / BRAVO_BLOCK_INTERVAL + 1 );
      ...
      {
         o.total_reward_shares2 = bravo::chain::util::evaluate_reward_curve( 10 );
      });
      ...
      BOOST_REQUIRE( mod_sam_comment.cashout_time == mod_sam_comment.created + BRAVO_CASHOUT_WINDOW_SECONDS );
      validate_database();
      ...
      generate_blocks( 60 * 5 / BRAVO_BLOCK_INTERVAL );
...
BOOST_AUTO_TEST_CASE( comment_delete_apply )
{
   try
   {
      ...
      tx.set_expiration( db.head_block_time() + BRAVO_MIN_TRANSACTION_EXPIRATION_LIMIT );
      tx.sign( alice_private_key, db.get_chain_id() );
      db.push_transaction( tx, 0 );
      ...
      tx.operations.push_back( op );
      tx.sign( alice_private_key, db.get_chain_id() );
      BRAVO_REQUIRE_THROW( db.push_transaction( tx, 0 ), fc::assert_exception );


      BOOST_TEST_MESSAGE( "--- Test success deleting a comment with negative rshares" );

      generate_block();
      vote.weight = -1 * BRAVO_100_PERCENT;
      tx.clear();
      tx.operations.push_back( vote );
      tx.operations.push_back( op );
      tx.sign( alice_private_key, db.get_chain_id() );
      ...
      generate_blocks( BRAVO_MIN_ROOT_COMMENT_INTERVAL.to_seconds() / BRAVO_BLOCK_INTERVAL );
      ...
      for (int i = 0; i < BRAVO_MAX_CASHOUT_WINDOWS; ++i) {
         fc::time_point_sec prevCashOutTime = db.get_comment("alice", string("test1")).cashout_time;
         generate_blocks(db.get_comment("alice", string("test1")).cashout_time, true);
         BOOST_REQUIRE(db.get_comment("alice", string("test1")).last_payout == db.head_block_time());
         ...
         {
            BOOST_REQUIRE(db.get_comment("alice", string("test1")).cashout_time == (prevCashOutTime + BRAVO_CASHOUT_WINDOW_SECONDS));
         }
      }
...
BOOST_AUTO_TEST_CASE( vote_apply )
{
   try
   {
...

      {
         ...
         int64_t max_vote_denom = ( db.get_dynamic_global_properties().vote_power_reserve_rate * BRAVO_VOTE_REGENERATION_SECONDS ) / (60*60*24);
         ...
         BOOST_REQUIRE( alice_comment.net_rshares.value == alice.effective_bravo_balance().amount.value * ( old_voting_power - alice.voting_power ) / BRAVO_100_PERCENT );
         BOOST_REQUIRE( alice_comment.cashout_time == alice_comment.created + BRAVO_CASHOUT_WINDOW_SECONDS );
         BOOST_REQUIRE( itr->rshares == alice.effective_bravo_balance().amount.value * ( old_voting_power - alice.voting_power ) / BRAVO_100_PERCENT );
         BOOST_REQUIRE( itr != vote_idx.end() );
         validate_database();

         BOOST_TEST_MESSAGE( "--- Test reduced power for quick voting" );

         generate_blocks( db.head_block_time() + BRAVO_MIN_VOTE_INTERVAL_SEC );

         old_voting_power = db.get_account( "alice" ).voting_power;

	     int64_t elapsed_seconds   = (db.head_block_time() - alice.last_vote_time).to_seconds();
         int64_t regenerated_power = (BRAVO_100_PERCENT * elapsed_seconds) / BRAVO_VOTE_REGENERATION_SECONDS;
         int64_t current_power     = std::min( int64_t(alice.voting_power + regenerated_power), int64_t(BRAVO_100_PERCENT) );
	     ...
	     auto alice_vpower = db.get_account( "alice" ).voting_power;

         BOOST_REQUIRE( db.get_account( "alice" ).voting_power == old_voting_power - ( ( old_voting_power + max_vote_denom - 1 ) * BRAVO_100_PERCENT / ( 2 * max_vote_denom * BRAVO_100_PERCENT ) ) );
         BOOST_REQUIRE( bob_comment.net_rshares.value == alice.effective_bravo_balance().amount.value * ( old_voting_power - db.get_account( "alice" ).voting_power ) / BRAVO_100_PERCENT );
         BOOST_REQUIRE( bob_comment.cashout_time == bob_comment.created + BRAVO_CASHOUT_WINDOW_SECONDS );
         BOOST_REQUIRE( itr != vote_idx.end() );
         validate_database();
         ...
         generate_blocks( db.head_block_time() + fc::seconds( ( BRAVO_CASHOUT_WINDOW_SECONDS / 2 ) ), true );
         ...
         uint128_t new_cashout_time = db.head_block_time().sec_since_epoch() + BRAVO_CASHOUT_WINDOW_SECONDS;

         BOOST_REQUIRE( new_bob.voting_power == BRAVO_100_PERCENT - ( ( BRAVO_100_PERCENT + max_vote_denom - 1 ) / max_vote_denom ) );
         BOOST_REQUIRE( new_alice_comment.net_rshares.value == old_abs_rshares + new_bob.effective_bravo_balance().amount.value * ( old_voting_power - new_bob.voting_power ) / BRAVO_100_PERCENT );
         BOOST_REQUIRE( new_alice_comment.cashout_time == new_alice_comment.created + BRAVO_CASHOUT_WINDOW_SECONDS );
         BOOST_REQUIRE( itr != vote_idx.end() );
         validate_database();
         ...
         op.weight = -1 * BRAVO_100_PERCENT / 2;
         op.voter = "sam";
         op.author = "bob";
         op.permlink = "foo";
         tx.operations.clear();
         ...
         new_cashout_time = db.head_block_time().sec_since_epoch() + BRAVO_CASHOUT_WINDOW_SECONDS;
         auto sam_weight /*= ( ( uint128_t( new_sam.vesting_shares.amount.value ) ) / 400 + 1 ).to_uint64();*/
                         = ( ( uint128_t( new_sam.effective_bravo_balance().amount.value ) * ( ( BRAVO_100_PERCENT + max_vote_denom - 1 ) / ( 2 * max_vote_denom ) ) ) / BRAVO_100_PERCENT ).to_uint64();

         BOOST_REQUIRE( new_sam.voting_power == BRAVO_100_PERCENT - ( ( BRAVO_100_PERCENT + max_vote_denom - 1 ) / ( 2 * max_vote_denom ) ) );
         ...
         BOOST_REQUIRE( new_bob_comment.cashout_time == new_bob_comment.created + BRAVO_CASHOUT_WINDOW_SECONDS );
         BOOST_REQUIRE( itr != vote_idx.end() );
         validate_database();
         ...
         regenerated_power = (BRAVO_100_PERCENT * ( db.head_block_time() - db.get_account( "alice").last_vote_time ).to_seconds() ) / BRAVO_VOTE_REGENERATION_SECONDS;
         ...
         auto new_rshares = ( ( fc::uint128_t( db.get_account( "alice" ).effective_bravo_balance().amount.value ) * used_power ) / BRAVO_100_PERCENT ).to_uint64();

         BOOST_REQUIRE( db.get_comment( "alice", string( "foo" ) ).cashout_time == db.get_comment( "alice", string( "foo" ) ).created + BRAVO_CASHOUT_WINDOW_SECONDS );
	     ...
	     elapsed_seconds   = (db.head_block_time() - new_alice.last_vote_time).to_seconds();
         regenerated_power = (BRAVO_100_PERCENT * elapsed_seconds) / BRAVO_VOTE_REGENERATION_SECONDS;
         current_power     = std::min( int64_t(new_alice.voting_power + regenerated_power), int64_t(BRAVO_100_PERCENT));

         used_power = ( ( BRAVO_1_PERCENT * 25 * ( current_power ) / BRAVO_100_PERCENT ) + max_vote_denom - 1 ) / max_vote_denom;
         auto alice_voting_power = current_power - used_power;

         op.voter = "alice";
         op.weight = BRAVO_1_PERCENT * 25;
         op.author = "bob";
         op.permlink = "foo";
         tx.operations.clear();
         tx.signatures.clear();
         ...
         new_rshares = ( ( fc::uint128_t( new_alice.effective_bravo_balance().amount.value ) * used_power ) / BRAVO_100_PERCENT ).to_uint64();
	     ...
	     new_alice = db.get_account( "alice" );
	     elapsed_seconds   = (db.head_block_time() - new_alice.last_vote_time).to_seconds();
         regenerated_power = (BRAVO_100_PERCENT * elapsed_seconds) / BRAVO_VOTE_REGENERATION_SECONDS;
         current_power     = std::min( int64_t(alice_voting_power + regenerated_power), int64_t(BRAVO_100_PERCENT));


         used_power = ( uint64_t( BRAVO_1_PERCENT ) * 75 * uint64_t( current_power ) ) / BRAVO_100_PERCENT;
         used_power = ( used_power + max_vote_denom - 1 ) / max_vote_denom;
         alice_voting_power = current_power - used_power;

         op.weight = BRAVO_1_PERCENT * -75;
         tx.operations.clear();
         tx.signatures.clear();
         tx.operations.push_back( op );
         tx.sign( alice_private_key, db.get_chain_id() );
         ...
         regenerated_power = (BRAVO_100_PERCENT * elapsed_seconds) / BRAVO_VOTE_REGENERATION_SECONDS;
         current_power     = std::min( int64_t(alice_voting_power + regenerated_power), int64_t(BRAVO_100_PERCENT));
	      alice_voting_power = current_power;
         ...
         generate_blocks( fc::time_point_sec( ( new_bob_comment.cashout_time - BRAVO_UPVOTE_LOCKOUT_HF17 ).sec_since_epoch() + BRAVO_BLOCK_INTERVAL ), true );
         ...
         BOOST_TEST_MESSAGE("--- Test success for multiple cashout" );
         
         comment_operation com_op;
         com_op.author = "sam";
         com_op.permlink = "another";
         com_op.parent_permlink = "test";
         com_op.title = "bar";
         com_op.body = "foo bar";
         tx.operations.clear();
         tx.signatures.clear();
         tx.operations.push_back(com_op);
         tx.set_expiration(db.head_block_time() + BRAVO_MAX_TIME_UNTIL_EXPIRATION);
         tx.sign(sam_private_key, db.get_chain_id());
         db.push_transaction(tx, 0);
         ...
         auto& sam_comment = db.get_comment("sam", string("another"));
         const auto& dave = db.get_account("dave");
         const auto& sam = db.get_account("sam");
         
         uint8_t nb_cashouts = sam_comment.max_cashout_windows;
         op.voter = "dave";
         op.author = "sam";
         op.permlink = "another";

         int64_t sam_com_old_net_rshares = 0;

         // voting on the same comment with different weight to test multiple cashout
         for (uint8_t max_cashout = nb_cashouts; max_cashout > 0; max_cashout--)
         {
            elapsed_seconds = (db.head_block_time() - dave.last_vote_time).to_seconds();
            regenerated_power = (BRAVO_100_PERCENT * elapsed_seconds) / BRAVO_VOTE_REGENERATION_SECONDS;
            current_power = std::min(int64_t(dave.voting_power + regenerated_power), int64_t(BRAVO_100_PERCENT));

            auto itr = vote_idx.find(std::make_tuple(sam_comment.id, dave.id));
            if (max_cashout != nb_cashouts)
               sam_com_old_net_rshares = itr->rshares;

            op.weight = (BRAVO_100_PERCENT/max_cashout);
            tx.operations.clear();
            tx.signatures.clear();
            tx.operations.push_back(op);
            tx.set_expiration(db.head_block_time() + BRAVO_MAX_TIME_UNTIL_EXPIRATION);
            ...
            itr = vote_idx.find(std::make_tuple(sam_comment.id, dave.id));
            
            BOOST_REQUIRE(itr != vote_idx.end());
            BOOST_REQUIRE(sam_comment.net_rshares.value == (dave.effective_bravo_balance().amount.value *
               (current_power - dave.voting_power) / BRAVO_100_PERCENT) - sam_com_old_net_rshares);
            BOOST_REQUIRE(sam_comment.cashout_time == sam_comment.created +
               ((nb_cashouts - max_cashout + 1) * BRAVO_CASHOUT_WINDOW_SECONDS));

            generate_blocks(fc::time_point_sec((sam_comment.cashout_time).sec_since_epoch()), true);

            BOOST_REQUIRE(sam_comment.net_rshares.value == 0);

            if (max_cashout > 1)
               BOOST_REQUIRE(sam_comment.cashout_time == sam_comment.created +
                  ((nb_cashouts - max_cashout + 2) * BRAVO_CASHOUT_WINDOW_SECONDS));
            else
...
BOOST_AUTO_TEST_CASE( transfer_to_vesting_apply )
{
   try
   {
      ...
      auto vests = asset( 0, BRAVO_SYMBOL );
      transfer_to_vesting_operation op;
      op.from = "alice";
      op.to = "";
      ...
      BOOST_REQUIRE( bob.balance.amount.value == ASSET( "0.000 TESTS" ).amount.value );
      BOOST_REQUIRE( 0 == vests.amount.value );
      BOOST_REQUIRE( 0 == shares.amount.value );
      validate_database();
   }
   FC_LOG_AND_RETHROW()
}
...
BOOST_AUTO_TEST_CASE( witness_update_apply )
{
   try
   {
      ...
      op.fee = ASSET( "1.000 TESTS" );
      op.block_signing_key = signing_key.get_public_key();
      op.props.account_creation_fee = asset( BRAVO_MIN_ACCOUNT_CREATION_FEE + 10, BRAVO_SYMBOL);
      op.props.maximum_block_size = BRAVO_MIN_BLOCK_SIZE_LIMIT + 100;
...
BOOST_AUTO_TEST_CASE( account_witness_proxy_apply )
{
   try
   {
      ...
      BOOST_REQUIRE( alice.proxy == BRAVO_PROXY_TO_SELF_ACCOUNT );
      validate_database();
      ...
      BOOST_REQUIRE( sam.proxy == BRAVO_PROXY_TO_SELF_ACCOUNT );
      validate_database();
      ...
      BOOST_REQUIRE( dave.proxy == BRAVO_PROXY_TO_SELF_ACCOUNT );
      validate_database();
      ...
      tx.operations.clear();
      tx.signatures.clear();
      op.proxy = BRAVO_PROXY_TO_SELF_ACCOUNT;
      op.account = "bob";
      tx.operations.push_back( op );
      tx.sign( bob_private_key, db.get_chain_id() );
      ...
      BOOST_REQUIRE( alice.proxy == "sam" );
      BOOST_REQUIRE( alice.proxied_vsf_votes_total().value == 0 );
      BOOST_REQUIRE( bob.proxy == BRAVO_PROXY_TO_SELF_ACCOUNT );
      BOOST_REQUIRE( bob.proxied_vsf_votes_total().value == 0 );
      BOOST_REQUIRE( sam.proxy == "dave" );
      BOOST_REQUIRE( dave.proxy == BRAVO_PROXY_TO_SELF_ACCOUNT );
      validate_database();
      ...
      vote.account= "bob";
      vote.witness = BRAVO_INIT_MINER_NAME;
      tx.operations.clear();
      tx.signatures.clear();
      tx.operations.push_back( vote );
      tx.sign( bob_private_key, db.get_chain_id() );
...
BOOST_AUTO_TEST_CASE( account_recovery )
{
   try
   {
      ...
      generate_blocks( db.head_block_time() + BRAVO_OWNER_UPDATE_LIMIT );
      ...
      generate_blocks( db.head_block_time() + BRAVO_OWNER_UPDATE_LIMIT + fc::seconds( BRAVO_BLOCK_INTERVAL ) );
      ...
      BOOST_REQUIRE( req_itr->expires == db.head_block_time() + BRAVO_ACCOUNT_RECOVERY_REQUEST_EXPIRATION_PERIOD );
      auto expires = req_itr->expires;
      ++req_itr;
      BOOST_REQUIRE( req_itr == request_idx.end() );

      generate_blocks( time_point_sec( expires - BRAVO_BLOCK_INTERVAL ), true );
      ...
      generate_blocks( db.head_block_time() + ( BRAVO_OWNER_AUTH_RECOVERY_PERIOD - BRAVO_ACCOUNT_RECOVERY_REQUEST_EXPIRATION_PERIOD ) );
      generate_block();
...
BOOST_AUTO_TEST_CASE( change_recovery_account )
{
   try
   {
      ...
      BRAVO_REQUIRE_THROW( change_recovery_account("alice", "nobody"), fc::exception );
      BRAVO_REQUIRE_THROW( change_recovery_account("haxer", "sam"   ), fc::exception );
      BRAVO_REQUIRE_THROW( change_recovery_account("haxer", "nobody"), fc::exception );
      ...
      generate_blocks( db.head_block_time() + BRAVO_OWNER_AUTH_RECOVERY_PERIOD - fc::seconds( BRAVO_BLOCK_INTERVAL ), true );
      // cannot request account recovery until recovery account is approved
      BRAVO_REQUIRE_THROW( request_account_recovery( "sam", sam_private_key, "alice", alice_pub1 ), fc::exception );
      generate_blocks(1);
      // cannot finish account recovery until requested
      BRAVO_REQUIRE_THROW( recover_account( "alice", alice_priv1, alice_private_key ), fc::exception );
      // do the request
...
BOOST_AUTO_TEST_CASE( escrow_transfer_validate )
{
   try
   {
      ...
      op.bravo_amount = ASSET( "1.000 TESTS" );
      op.escrow_id = 0;
      op.agent = "sam";
      ...
      op.sbd_amount.symbol = BRAVO_SYMBOL;
      BRAVO_REQUIRE_THROW( op.validate(), fc::exception );

      BOOST_TEST_MESSAGE( "--- failure when bravo symbol != BRAVO" );
      op.sbd_amount.symbol = SBD_SYMBOL;
      op.bravo_amount.symbol = SBD_SYMBOL;
      BRAVO_REQUIRE_THROW( op.validate(), fc::exception );

      BOOST_TEST_MESSAGE( "--- failure when fee symbol != SBD and fee symbol != BRAVO" );
      op.bravo_amount.symbol = BRAVO_SYMBOL;
      op.fee.symbol = VESTS_SYMBOL;
      BRAVO_REQUIRE_THROW( op.validate(), fc::exception );

      BOOST_TEST_MESSAGE( "--- failure when sbd == 0 and bravo == 0" );
      op.fee.symbol = BRAVO_SYMBOL;
      op.sbd_amount.amount = 0;
      op.bravo_amount.amount = 0;
      BRAVO_REQUIRE_THROW( op.validate(), fc::exception );

      BOOST_TEST_MESSAGE( "--- failure when sbd < 0" );
      op.sbd_amount.amount = -100;
      op.bravo_amount.amount = 1000;
      BRAVO_REQUIRE_THROW( op.validate(), fc::exception );

      BOOST_TEST_MESSAGE( "--- failure when bravo < 0" );
      op.sbd_amount.amount = 1000;
      op.bravo_amount.amount = -100;
      BRAVO_REQUIRE_THROW( op.validate(), fc::exception );
...
BOOST_AUTO_TEST_CASE( escrow_transfer_apply )
{
   try
   {
      ...
      auto alice_bravo_balance = alice.balance - op.bravo_amount - op.fee;
      auto alice_sbd_balance = op.sbd_amount;
      auto bob_bravo_balance = bob.balance;
      auto sam_bravo_balance = sam.balance;
      ...
      BOOST_REQUIRE( escrow.bravo_balance == op.bravo_amount );
      ...
      BOOST_REQUIRE( alice.balance == alice_bravo_balance );
      BOOST_REQUIRE( bob.balance == bob_bravo_balance );
      BOOST_REQUIRE( sam.balance == sam_bravo_balance );
...
BOOST_AUTO_TEST_CASE( escrow_approve_apply )
{
   try
   {
      ...
      et_op.to = "bob";
      et_op.agent = "sam";
      et_op.bravo_amount = ASSET( "1.000 TESTS" );
      ...
      BOOST_REQUIRE( escrow.bravo_balance == ASSET( "1.000 TESTS" ) );
      ...
      tx.set_expiration( db.head_block_time() + BRAVO_BLOCK_INTERVAL );
      tx.sign( bob_private_key, db.get_chain_id() );
      BRAVO_REQUIRE_THROW( db.push_transaction( tx, 0 ), fc::exception );
      ...
      BRAVO_REQUIRE_THROW( db.get_escrow( op.from, op.escrow_id ), fc::exception );
      BOOST_REQUIRE( alice.balance == ASSET( "10.000 TESTS" ) );
      validate_database();
      ...
      generate_blocks( et_op.ratification_deadline + BRAVO_BLOCK_INTERVAL, true );
...
BOOST_AUTO_TEST_CASE( escrow_dispute_apply )
{
   try
   {
      ...
      et_op.bravo_amount = ASSET( "1.000 TESTS" );
      et_op.fee = ASSET( "0.100 TESTS" );
      et_op.ratification_deadline = db.head_block_time() + BRAVO_BLOCK_INTERVAL;
      et_op.escrow_expiration = db.head_block_time() + 2 * BRAVO_BLOCK_INTERVAL;
      ...
      BOOST_REQUIRE( escrow.ratification_deadline == et_op.ratification_deadline );
      BOOST_REQUIRE( escrow.escrow_expiration == et_op.escrow_expiration );
      BOOST_REQUIRE( escrow.bravo_balance == et_op.bravo_amount );
...
BOOST_AUTO_TEST_CASE( escrow_release_validate )
{
   try
   {
      ...
      BOOST_TEST_MESSAGE( "--- failure when bravo == 0 and sbd == 0" );
      op.sbd_amount.amount = 0;
      BRAVO_REQUIRE_THROW( op.validate(), fc::exception );
      ...
      BOOST_TEST_MESSAGE( "--- failure when bravo is not bravo symbol" );
      op.sbd_amount.symbol = SBD_SYMBOL;
      op.bravo_amount = ASSET( "1.000 TBD" );
      BRAVO_REQUIRE_THROW( op.validate(), fc::exception );
...
BOOST_AUTO_TEST_CASE( escrow_release_apply )
{
   try
   {
      ...
      op.bravo_amount = ASSET( "0.100 TESTS" );
      ...
      BOOST_REQUIRE( db.get_escrow( op.from, op.escrow_id ).bravo_balance == ASSET( "0.900 TESTS" ) );
      ...
      BOOST_REQUIRE( db.get_escrow( op.from, op.escrow_id ).bravo_balance == ASSET( "0.800 TESTS" ) );
      ...
      BOOST_TEST_MESSAGE( "--- failure when releasing less bravo than available" );
      op.bravo_amount = ASSET( "0.000 TESTS" );
      op.sbd_amount = ASSET( "1.000 TBD" );
      ...
      BOOST_REQUIRE( db.get_escrow( et_op.from, et_op.escrow_id ).bravo_balance == ASSET( "0.700 TESTS" ) );
      ...
      BOOST_REQUIRE( db.get_escrow( et_op.from, et_op.escrow_id ).bravo_balance == ASSET( "0.600 TESTS" ) );
      ...
      BOOST_REQUIRE( db.get_escrow( et_op.from, et_op.escrow_id ).bravo_balance == ASSET( "0.500 TESTS" ) );


      BOOST_TEST_MESSAGE( "--- success deleting escrow when balances are both zero" );
      tx.clear();
      op.bravo_amount = ASSET( "0.500 TESTS" );
      tx.operations.push_back( op );
      tx.sign( sam_private_key, db.get_chain_id() );
      db.push_transaction( tx, 0 );

      BOOST_REQUIRE( db.get_account( "alice" ).balance == ASSET( "9.700 TESTS" ) );
      BRAVO_REQUIRE_THROW( db.get_escrow( et_op.from, et_op.escrow_id ), fc::exception );
      ...
      BOOST_REQUIRE( db.get_escrow( et_op.from, et_op.escrow_id ).bravo_balance == ASSET( "0.900 TESTS" ) );
      ...
      BOOST_REQUIRE( db.get_escrow( et_op.from, et_op.escrow_id ).bravo_balance == ASSET( "0.800 TESTS" ) );
      ...
      tx.clear();
      op.bravo_amount = ASSET( "0.600 TESTS" );
      tx.operations.push_back( op );
      tx.sign( alice_private_key, db.get_chain_id() );
      db.push_transaction( tx, 0 );
...
BOOST_AUTO_TEST_CASE( transfer_to_savings_apply )
{
   try
   {
      ...
      BOOST_TEST_MESSAGE( "--- success transferring BRAVO to self" );
      op.to = "alice";
      ...
      BOOST_TEST_MESSAGE( "--- success transferring BRAVO to other" );
      op.to = "bob";
      op.amount = ASSET( "1.000 TESTS" );
...
BOOST_AUTO_TEST_CASE( transfer_from_savings_apply )
{
   try
   {
      ...
      BOOST_TEST_MESSAGE( "--- success withdrawing BRAVO to self" );
      op.to = "alice";
      ...
      BOOST_REQUIRE( db.get_savings_withdraw( "alice", op.request_id ).complete == db.head_block_time() + BRAVO_SAVINGS_WITHDRAW_TIME );
      validate_database();
      ...
      BOOST_TEST_MESSAGE( "--- success withdrawing BRAVO to other" );
      op.to = "bob";
      op.amount = ASSET( "1.000 TESTS" );
      op.request_id = 3;
      ...
      generate_blocks( db.head_block_time() + BRAVO_SAVINGS_WITHDRAW_TIME - fc::seconds( BRAVO_BLOCK_INTERVAL ), true );
      ...
      for( int i = 0; i < BRAVO_SAVINGS_WITHDRAW_REQUEST_LIMIT; i++ )
      {
         op.request_id = i;
         tx.clear();
         tx.operations.push_back( op );
      ...
      op.request_id = BRAVO_SAVINGS_WITHDRAW_REQUEST_LIMIT;
      tx.clear();
      tx.operations.push_back( op );
      tx.sign( alice_private_key, db.get_chain_id() );
      BRAVO_REQUIRE_THROW( db.push_transaction( tx, 0 ), fc::exception );
      BOOST_REQUIRE( db.get_account( "alice" ).savings_withdraw_requests == BRAVO_SAVINGS_WITHDRAW_REQUEST_LIMIT );
      validate_database();
   }
   FC_LOG_AND_RETHROW()
}
...
BOOST_AUTO_TEST_CASE( decline_voting_rights_apply )
{
   try
   {
      ...
      BOOST_REQUIRE( itr->effective_date == db.head_block_time() + BRAVO_OWNER_AUTH_RECOVERY_PERIOD );
      ...
      vote.weight = BRAVO_1_PERCENT * 50;
      tx.clear();
      tx.operations.push_back( vote );
      tx.sign( alice_private_key, db.get_chain_id() );
      BRAVO_REQUIRE_THROW( db.push_transaction( tx, 0 ), fc::exception );
...
BOOST_AUTO_TEST_CASE( account_bandwidth )
{
   try
   {
      ...
      BOOST_REQUIRE( average_bandwidth == fc::raw::pack_size( tx ) * 10 * BRAVO_BANDWIDTH_PRECISION );
      ...
      BOOST_REQUIRE( last_bandwidth_update == db.head_block_time() );
      BOOST_REQUIRE( average_bandwidth == total_bandwidth + fc::raw::pack_size( tx ) * 10 * BRAVO_BANDWIDTH_PRECISION );
   }
   FC_LOG_AND_RETHROW()
}
...
BOOST_AUTO_TEST_CASE( claim_reward_balance_validate )
{
   try
   {
      ...
      op.account = "alice";
      op.reward_bravo = ASSET( "0.000 TESTS" );

      BOOST_TEST_MESSAGE( "Testing all 0 amounts" );
      BRAVO_REQUIRE_THROW( op.validate(), fc::assert_exception );


      BOOST_TEST_MESSAGE( "Testing single reward claims" );
      op.reward_bravo.amount = 1000;
      op.validate();

      BOOST_TEST_MESSAGE( "Testing wrong BRAVO symbol" );
      op.reward_bravo = ASSET( "1.000 WRONG" );
      BRAVO_REQUIRE_THROW( op.validate(), fc::assert_exception );
   }
   FC_LOG_AND_RETHROW()
}
...
BOOST_AUTO_TEST_CASE( claim_reward_balance_apply )
{
   try
   {
      ...
      db_plugin->debug_update( []( database& db )
      {
         ...
         {
            a.reward_bravo_balance += ASSET( "10.000 TESTS" );
         });

         db.modify( db.get_dynamic_global_properties(), []( dynamic_global_property_object& gpo )
         {
            gpo.current_supply += ASSET( "10.000 TESTS" );
         });
      });
      ...
      auto alice_bravo = db.get_account( "alice" ).balance;

      BOOST_TEST_MESSAGE( "--- Attempting to claim more BRAVO than exists in the reward balance." );

      claim_reward_balance_operation op;
      signed_transaction tx;

      op.account = "alice";
      op.reward_bravo = ASSET( "20.000 TESTS" );
      ...
      asset reward_bravo_balance = alice_acc.reward_bravo_balance;

      op.reward_bravo = ASSET( "10.000 TESTS" );
      tx.clear();
      tx.operations.push_back( op );
      tx.sign( alice_private_key, db.get_chain_id() );
      db.push_transaction( tx, 0 );

      BOOST_REQUIRE( db.get_account( "alice" ).balance == alice_bravo + op.reward_bravo );
      BOOST_REQUIRE( db.get_account( "alice" ).reward_bravo_balance == (reward_bravo_balance - ASSET("10.000 TESTS") ) );
      validate_database();
   }
   FC_LOG_AND_RETHROW()
}
...
BOOST_AUTO_TEST_CASE( comment_beneficiaries_validate )
{
   try
   {
      ...
      b.beneficiaries.push_back( beneficiary_route_type( account_name_type( "bob" ), BRAVO_100_PERCENT + 1 ) );
      op.extensions.insert( b );
      BRAVO_REQUIRE_THROW( op.validate(), fc::assert_exception );

      BOOST_TEST_MESSAGE( "--- Testing more than 100% total weight" );
      b.beneficiaries.clear();
      b.beneficiaries.push_back( beneficiary_route_type( account_name_type( "bob" ), BRAVO_1_PERCENT * 75 ) );
      b.beneficiaries.push_back( beneficiary_route_type( account_name_type( "sam" ), BRAVO_1_PERCENT * 75 ) );
      op.extensions.clear();
      op.extensions.insert( b );
      BRAVO_REQUIRE_THROW( op.validate(), fc::assert_exception );
      ...
      b.beneficiaries.clear();
      b.beneficiaries.push_back( beneficiary_route_type( "bob", BRAVO_1_PERCENT * 2 ) );
      b.beneficiaries.push_back( beneficiary_route_type( "bob", BRAVO_1_PERCENT ) );
      op.extensions.clear();
      op.extensions.insert( b );
      BRAVO_REQUIRE_THROW( op.validate(), fc::assert_exception );
      ...
      b.beneficiaries.clear();
      b.beneficiaries.push_back( beneficiary_route_type( "bob", BRAVO_1_PERCENT ) );
      b.beneficiaries.push_back( beneficiary_route_type( "alice", BRAVO_1_PERCENT ) );
      op.extensions.clear();
      op.extensions.insert( b );
      BRAVO_REQUIRE_THROW( op.validate(), fc::assert_exception );
...
BOOST_AUTO_TEST_CASE( comment_beneficiaries_apply )
{
   try
   {
      ...
      b.beneficiaries.push_back( beneficiary_route_type( account_name_type( "bob" ), BRAVO_1_PERCENT ) );

      for( size_t i = 0; i < 8; i++ )
      {
         b.beneficiaries.push_back( beneficiary_route_type( account_name_type( BRAVO_INIT_MINER_NAME + fc::to_string( i ) ), BRAVO_1_PERCENT ) );
      }
      ...
      tx.operations.push_back( op );
      tx.sign( alice_private_key, db.get_chain_id() );
      BRAVO_REQUIRE_THROW( db.push_transaction( tx ), chain::plugin_exception );


      BOOST_TEST_MESSAGE( "--- Test specifying a non-existent benefactor" );
      b.beneficiaries.clear();
      b.beneficiaries.push_back( beneficiary_route_type( account_name_type( "doug" ), BRAVO_1_PERCENT ) );
      op.extensions.clear();
      op.extensions.insert( b );
      tx.clear();
      tx.operations.push_back( op );
      tx.sign( alice_private_key, db.get_chain_id() );
      BRAVO_REQUIRE_THROW( db.push_transaction( tx ), fc::assert_exception );

      asset bob_prev = db.get_account("bob").reward_bravo_balance;
      asset sam_prev = db.get_account("sam").reward_bravo_balance;
      ...
      b.beneficiaries.clear();
      b.beneficiaries.push_back( beneficiary_route_type( account_name_type( "bob" ), 25 * BRAVO_1_PERCENT ) );
      b.beneficiaries.push_back( beneficiary_route_type( account_name_type( "sam" ), 50 * BRAVO_1_PERCENT ) );
      op.extensions.clear();
      op.extensions.insert( b );
      ...
      b.beneficiaries.clear();
      b.beneficiaries.push_back( beneficiary_route_type( account_name_type( "dave" ), 25 * BRAVO_1_PERCENT ) );
      op.extensions.clear();
      op.extensions.insert( b );
      tx.sign( alice_private_key, db.get_chain_id() );
      BRAVO_REQUIRE_THROW( db.push_transaction( tx ), fc::assert_exception );
      ...
      generate_blocks( db.get_comment( "alice", string( "test" ) ).cashout_time - BRAVO_BLOCK_INTERVAL );

      db_plugin->debug_update( [=]( database& db )
      {
         db.modify( db.get_dynamic_global_properties(), [=]( dynamic_global_property_object& gpo )
         {
            gpo.current_supply -= gpo.total_reward_fund_bravo;
            gpo.total_reward_fund_bravo = ASSET( "100.000 TESTS" );
            gpo.current_supply += gpo.total_reward_fund_bravo;
         });
      });

      generate_block();
      BOOST_REQUIRE(db.get_account("bob").reward_bravo_balance.amount + db.get_account("sam").reward_bravo_balance.amount - bob_prev.amount - sam_prev.amount  
